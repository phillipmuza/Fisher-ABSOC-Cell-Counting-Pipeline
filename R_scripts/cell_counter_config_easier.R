#### GENERIC SCRIPT TO CALCULATE VOLUMES AND ANNOTATE CELLS IN APPROPRIATE
      #### BRAIN REGIONS - THIS SCRIPT NEEDS YOU TO MANUALLY 
          #### CHANGE YOUR REGIONS ####
...
### 5. AMMEND NEEDS YOUR INPUT - Functions to slice Data Tree by Structure_ID_Path ID  --------
## ADD THE REGIONS YOU ARE INTERESTED HERE ##
# Assign the structure_id_path to the structure name #

HPC_formation <- "/997/8/567/688/695/1089"
Dentate_Gyrus <- "/997/8/567/688/695/1089/1080/726/"

#This function will filter your ROI from the list of anatomical regions in the brain slice
#This filter is based on the structures you are interested in
structure_slice_tree <- function(data, tree_structure, save_filename) {
  df <- data %>% 
    filter(str_detect(structure_id_path, tree_structure))
  df$parent_structure_id <- as.factor(df$parent_structure_id)
  df <- df %>% 
    adorn_totals("row")
  write.csv(df, file =  save_filename)
...
#Enter below the regions you are interested in below
#You will need the "combined_cells_volumes" as the first entry, 
#then what your structure is called, and what to save it as

structure_slice_tree(
  combined_cells_volumes, HPC_formation, "HPC_formation.csv")

structure_slice_tree(
  combined_cells_volumes, Dentate_Gyrus, "Dentate_Gyrus.csv")
